...
def main(args):
    ...
    load_encoder = args['meta']['load_encoder']
    encoder_path = args['meta']['encoder_path']
    ...
    tensorboard_dir = os.path.join(args['logging']['tensorboard_dir'], f'{tag}')
    # ----------------------------------------------------------------------- #
    ...
    for epoch in range(start_epoch, num_epochs):
        ...
        for itr, fdata in enumerate(unsupervised_loader):

            def load_imgs():
                # -- unsupervised imgs
                imgs = [u.to(device, non_blocking=True) for u in fdata[0]]
                return imgs
            ...
            def log_stats():
                ...
                tot_itr = epoch * len(unsupervised_loader) + itr
                writer.add_scalar('loss/train', loss, tot_itr)
                writer.add_scalar('ploss/train', ploss, tot_itr)
                writer.add_scalar('rloss/train', rloss, tot_itr)
                writer.add_scalar('eloss/train', eloss, tot_itr)

                writer.add_scalar('avg_loss/train', loss_meter.avg, tot_itr)
                writer.add_scalar('avg_ploss/train', ploss_meter.avg, tot_itr)
                writer.add_scalar('avg_rloss/train', rloss_meter.avg, tot_itr)
                writer.add_scalar('avg_eloss/train', eloss_meter.avg, tot_itr)

                writer.add_scalar('LR/train', _new_lr, tot_itr)
        ...
        writer.add_scalar('epoch_loss/train', loss_meter.avg, epoch + 1)
...
def load_pretrained(
    ...
    logger.info(f'loading encoder from {pretrained}')
...
def init_model(
    ...
    if load_encoder:
        encoder = load_pretrained(encoder=encoder, pretrained=encoder_path)
