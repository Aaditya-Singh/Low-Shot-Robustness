...
# -- model class for in21k vit from https://github.com/huggingface/transformers/
class VisionTransformerWithPooling(nn.Module):
    def __init__(self, encoder, embed_dim=768):
        super().__init__()
...
def deit_base_with_pooling(patch_size=16, **kwargs):
    encoder = VisionTransformer(
        patch_size=patch_size, embed_dim=768, depth=12, num_heads=12, mlp_ratio=4,
        qkv_bias=True, norm_layer=partial(nn.LayerNorm, eps=1e-6), **kwargs)
    model = VisionTransformerWithPooling(encoder, embed_dim=768)
    return model
